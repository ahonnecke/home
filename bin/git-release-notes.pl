...
my $filename = "release_notes_$before.txt";
my $dataCommand = "git log --pretty=format:'-- %B'  --reverse --all --date=short --before={$before} --after={$after} | grep -v Merge | grep -v onflict | grep -v evert| sort | uniq > $filename";
